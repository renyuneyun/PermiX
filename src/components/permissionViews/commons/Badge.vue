...
const permissionTypeMap = new Map<string, PermissionType>(
  Object.values(PermissionType).map((v) => [v, v]),
);
...
const icon = computed(() =>
  props.mode ? "mdi-checkbox-marked-circle" : "mdi-cancel",
);
const text = computed(
  () => permissionText[props.type.toLowerCase() as keyof {}],
);
</script>
  ...
  <v-tooltip :text="props.type" location="top center" origin="auto">
    <template v-slot:activator="{ props }">
      <v-chip v-bind="props" :color="color"
        >{{ text }}<v-icon end :icon="icon"></v-icon
      ></v-chip>
    </template>
  </v-tooltip>
</template>
