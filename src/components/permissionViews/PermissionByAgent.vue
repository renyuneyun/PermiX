/* This component accepts `permission` as a prop, which is a `PermissionByAgent`
object. If not provided, it will use pinia store's `permissionByAgent`. */

<script setup lang="ts">
import { computed, reactive, watchEffect, ref } from "vue";
import {
  retrievePermissionByAgent,
  setPermission,
...
const url = computed(() => {
  ...
  else return explorerStore.currDir;
});
...
async function load() {
  ...
  try {
    ...
    await retrievePermissionByAgent(
      url.value,
      store.permission,
      store.progress,
...
async function onReceiveNewPermission(
  resources: string[],
  agent: string,
...
<template>
  <HighlightSegment>
    ...
    <template v-if="store.permission">
      <template
        v-for="([agent, resourceOfPermissionList], index0) in Object.entries(
          store.permission,
        )"
      >
        <HighlightSegment>
          ...
          <template
            v-for="(resourceOfPermission, index) in resourceOfPermissionList"
            :key="index"
          >
            <v-container>
              ...
              <v-container>
                <template
                  v-for="(res, index) in resourceOfPermission.resources"
                  :key="index"
                >
              ...
              <v-divider
                v-if="index < resourceOfPermissionList.length - 1"
              ></v-divider>

              <v-dialog
                v-model="dialog[dialogKey(index0, index)]"
                activator="parent"
              >
                <PermissionEditor
                  :agent="agent"
                  :permission="resourceOfPermission.permission"
                  @ok="
                    (agent, newPerm) => {
                      onReceiveNewPermission(
                        resourceOfPermission.resources,
                        agent,
                        newPerm,
                      );
                      dialog[dialogKey(index0, index)] = false;
                    }
                  "
                ></PermissionEditor>
              </v-dialog>
